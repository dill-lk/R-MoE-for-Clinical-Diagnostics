...
impl From<&ChatCompletionMessage> for GroqMessage {
    fn from(msg: &ChatCompletionMessage) -> Self {
        let content = match &msg.content {
            MessageContent::Text(text) => text.clone(),
            MessageContent::Multimodal(_) => "[Multimodal content not supported]".to_string(),
        };
        Self {
            role: msg.role.clone(),
            content,
...
impl ProviderClient for GroqClient {
    async fn chat_completion(
        ...
        debug!(model = %self.config.model, "Sending Groq request");
        ...
        let body: GroqResponse = response.json().await.map_err(|e| {
            RMoEError::ParseError(format!("Failed to parse Groq response: {}", e))
        })?;
    ...
    async fn vision_completion(
        ...
        Err(RMoEError::NotImplemented("Groq does not support vision".to_string()))
    }
